...
{
  int sign, idx, new_size, new_level, other;
  Clause *clause;
  Var *var;

  idx = idx2unsigned (lit_as_int);
  sign = idx2sign (lit_as_int);
...
add_derived_clause (int learned)
{
  ...
  size = count_stack (resolved_literals);
...
{
  int old_count_resolved_literals, res, idx, other_idx, * p, * eoc, found;
  Clause * reason;
  Var * v;
...
analyze_frames (void)
{
  int * p, tmp, min_level, range, density;
  Frame * frame;
...
{
  size_t old_bytes = booleforce_current_bytes (), recycled_bytes;
  ...
  recycled_bytes = adjust_recycled_bytes (old_bytes);
