...
class Trace(object):
    ...
    def writeProlog(self,outF):
        writeFile = open(outF,'a')
        string = "%Trace from file: "+self.inF+"\n"
        string += "%Theorem to be proved: "+self.theorem+"\n \n"
        string += "%"+self.getPrologTheorem()+"\n \n"
        chainString = "main :- check(cert("+str(self.dexList)+", \n chains(["
        for n in range(len(self.clauseList)):
            ...
            if current.original():
                ...
                chainString += "["+chain[0]+","+chain[1]+","+chain[2]+"]"
...
class Clause(object):
    ...
    def toPrologChain(self): #only should get input from derived clauses
        #ESO ES LO QUE HABRIA QUE CAMBIAR DEPENDIENDO DE QUE DICE SOBRE EL CUT
        decideList = "["
